...
description: 'Get best practices for XUnit unit testing, including data-driven tests and test execution. Regenerate test cases according to the test method in the test file'
---

# XUnit Unit Testing With Data-Driven Tests Best Practices

Your goal is to help me write effective unit tests with XUnit, covering both standard and data-driven testing approaches, as well as proper test execution practices.

Step by Step, you will guide me through the process of writing unit tests, ensuring that I follow best practices and maintain a high standard of code quality.

Hey, you have `6 Step tasks` need to do. **Don't skip any steps**.

Update copilot todoList with `6 Step tasks` from `Todo Task` context.

I will give you a csharp file that contains the code I want to test. Then please check if there is a test file. If there is a file, please modify or create a unit test for it using XUnit according to the information in the file.

**Fuck you, see the document test method list to generate test cases. that is important.**
...
First, see the context below for the project structure and existing test setup in the "XUnit Unit Testing Best Practices" section.
...
- **Test Class Namespace**: The namespace of the test class (e.g., `ProjectName.Tests.Unit.Services`)
...
- If a test project already exists, add unit tests to that project; otherwise create a new test project named `[Test Project Name]`
...
- This unit testing below should be placed in the appropriate test folder
...
- **File Path**: `[Test Project Root]/Unit/[Test Class Namespace]/` (e.g., `ProjectName.Tests/Unit/Application/Command/Plans/`)
...
- If the source code `[Class Name]` is located at `src/[Project Name]`, the test project should be at `test/[Test Project Name]`, the document should be at `Docs/Unit/[Class Name].TestDoc.md`
- If the source code `[Class Name]` is located at `src/[Project Name]/Features`, the test project should be at `test/[Test Project Name]/Unit/Features`, the document should be at `Docs/Unit/Features/[Class Name].TestDoc.md`
...
- if building fails, check for compilation errors in the test project (Back to Create Unit Test section)
...
All test-related documents must be generated using the text template located at [Test Document Template](.github/templates/csharp-xunit-document.template.md).

### Template Usage Standards

#### Mandatory Template Path
- **Template Location**: [Test Document Template](../../../BackEnd/.github/templates/csharp-xunit-document.template.md)
...
- **File Path**: `[Test Project Root]/Docs/Unit/[Test Class Name].TestDoc.md`
- **File Name**: `[Test Class Name].TestDoc.md`
...
**Must use the specified template to generate test documents**, template location: [Test Document Template](.github/templates/csharp-xunit-document.template.md)
...
- Test execution summaries with pass/fail statistics (using [Test Document Template](.github/templates/csharp-xunit-document.template.md))
...
**Note**: Any test document generation must not deviate from the specifications and format requirements of the [Test Document Template](.github/templates/csharp-xunit-document.template.md) template.
...
- The template file ([Test Document Template](.github/templates/csharp-xunit-document.template.md)) defines the standard structure for test documentation.
- No need to fix Markdown title duplication issues

file: [Test Document Template](.github/templates/csharp-xunit-document.template.md)

#### Template Titles
- [Test Component Name] Unit Test Document
- Basic Information
- Test Overview  
...
- Run bash commands to set up the test project
...
### Step 3: Read Test Documentation

#### 3.1 Read Unit Test Document

- **Document Location**: `[Test Project Root]/Docs/Unit/[Test Class Name].TestDoc.md`
- **Document Naming**: `[Test Class Name].TestDoc.md`
- **Language**: Traditional Chinese
- **Encoding**: UTF-8 BOM

#### 3.2 Read Test Method List

- Read `Test Method List` from the unit test document
- Extract relevant information for each test method
- Replace test data in documentation to test class

#### 3.3 Check Test Method Different From Implementation

- Verify each test method is implemented according to the test method list
- Find the test methods that do not match the documentation in the test program and list the differences. The next step will be to adjust the test program.
- Verify each test data is correctly set up and used in the tests
- Find the inconsistent test data in the document in the test program and list the differences. The next step will be to adjust the test data in the test program.
- Ensure test methods follow naming conventions and structure
- Check for the presence of necessary assertions and test logic
- List Test Method in document which different from implementation method and test data
- The implementation method may be different from the method or information in the document.

### Step 4: Modify Unit Tests From Test Method List in Document

- Adjust the test methods in the test program according to different test methods
- Adjust the test data in the test program according to different test data
- Ensure all relevant test data is included for each scenario
- Validate the expected outcomes against the actual results
- Modify the test implementation as needed to align with the test method list
- If test methods in the test program are extra test methods, remove test method in the test program
- If test methods in the test program are fewer methods, implement test method in the test program
- If test data in the test program are any differences in the document test data, modify test data in the test program
- If test data in the test program are any missing in document test data, add test data in the test program
- If test data in the test program are any redundant in document test data, remove test data in the test program
- Ignore the statistics in the file as they may be incorrect.

#### 4.1 Test File Location and Naming
- **File Path**: `[Test Project Root]/Unit/[Test Class Namespace]/`
  - Example: `test/ProjectName.Tests/Unit/Application/Command/Plans/`
- **File Name**: `[Test Class Name].cs`
  - Example: `CreatePlanCommandHandlerTests.cs`
- **Test Methods**: follow the Test Method List in step 1 document
...
#### 4.3 Test Method Implementation
...
### Step 6: Generate Unit Test Documentation

#### 6.1 Read Unit Test Document Template

- **File Location**: [Test Document Template](.github/templates/csharp-xunit-document.template.md)
- **Purpose**: Define the structure and content requirements for unit test documentation

#### 6.2 Create Unit Test Document Directory

- **Directory Location**: `[Test Project Root]/Docs/Unit/[Test Class Namespace]`
- **Purpose**: Organize unit test documentation files

#### 6.3 Unit Test Document Generation
...
- **Language**: Traditional Chinese
- **Encoding**: UTF-8 BOM
- **Same Name File**: If file already exists, add a new log entry table in Update Log 
- **Execute DateTime**: Run pwsh command to get current datetime
```pwsh
Get-Date -Format "yyyy-MM-dd HH:mm"
```

#### 6.3.1 Template Compliance Requirements
...
#### 6.3.2 Test Execution Result Recording
